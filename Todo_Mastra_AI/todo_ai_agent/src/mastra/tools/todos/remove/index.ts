...
import { RemoveTodosApiService } from "./services";
import { z } from "zod";

const removeTodosApiService=new RemoveTodosApiService();

export const removeTodoTools= createTool({
    id:"remove-todo",
    description:`
    ...
    execute:async ({context})=>{
        ...
        return {
            message: String(`Successfully requested to removed a todo item with identifier "${identifier}".`)
        }
    }
})